...
int main(void)
{
  ...
  Button_Init(&btnStart, GPIOC, GPIO_PIN_11);
  Button_Init(&btnStop, GPIOC, GPIO_PIN_12);
  */
  Button_Init(&btnRunStop, GPIOC, GPIO_PIN_10);
  Button_Init(&btnClear, GPIOC, GPIO_PIN_11);

  HAL_TIM_Base_Start_IT(&htim2); // Raise interrupt and delay user setting time
  /* USER CODE END 2 */
...
//			  HAL_UART_Transmit(&huart2, "hello STM32\n", strlen("hello STM32\n"), 1000);
	  ...
	  */
	  ...
	  /*
	  if (Button_GetState(&btnPower) == ACT_RELEASED) {
		  // power
	  }
	  if (Button_GetState(&btnStart) == ACT_RELEASED) {
		  // start
	  }
